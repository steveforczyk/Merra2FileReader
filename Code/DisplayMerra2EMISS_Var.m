function  DisplayMerra2EMISS_Var(titlestr)
% Display the surface emissivity variance
...
fprintf(fid,'%s\n','------- Plot The Variance Of The Surface Emissivity ------');
EMIS=VarEMISS.values;
...
elseif(iSubMean==1)
    ...
    maxval2=maxval+.1;
    dinc=(maxval-minval)/100;
    units=strcat('emissivity-',num2str(startmean,5));
end
...
txtstr2=strcat('1 ptile =',num2str(val01,6),'//-50 ptile =',num2str(val50,6),...
    '//-99 ptile=',num2str(val99,6),'-',units);
txt2=text(tx2,ty2,txtstr2,'FontWeight','bold','FontSize',12);
tx3=.10;
ty3=.10;
if(iSubMean==1)
    txtstr3=strcat('Plotted quantity is emissivity-',num2str(startmean,5));
