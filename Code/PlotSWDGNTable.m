function PlotSWDGNTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function will plot the SWGDN Table Values
...
plot(SWGDNTT.Time,SWGDNTT.SWGDN10,'b',SWGDNTT.Time,SWGDNTT.SWGDN50,'g',...
    SWGDNTT.Time,SWGDNTT.SWGDN70,'c',SWGDNTT.Time,SWGDNTT.SWGDN90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('SWDGN 10 ptile','SWDGN 50 ptile','SWDGN 70 ptile','SWDGN 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='SWDGN';
ylabel('Monthly surface incoming short wave flux -W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('SWDGN','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' SWDGN Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of the long wave uowelling dlux at the toa.';
