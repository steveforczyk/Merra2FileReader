function PlotLWUPTCLRCLNTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function will plot the LWUPTCLRCLN Table Values
...
global LWGABTable LWGABTT LWTUPTable LWTUPTT LWTUPCLRTable LWTUPCLRTT;
global LWTUPCLRCLNTable LWTUPCLRCLNTT;
...
plot(LWTUPCLRCLNTT.Time,LWTUPCLRCLNTT.LWTUPCLRCLN10,'b',LWTUPCLRCLNTT.Time,LWTUPCLRCLNTT.LWTUPCLRCLN50,'g',...
    LWTUPCLRCLNTT.Time,LWTUPCLRCLNTT.LWTUPCLRCLN70,'c',LWTUPCLRCLNTT.Time,LWTUPCLRCLNTT.LWTUPCLRCLN90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('LWTUPCLRCLN 10 ptile','LWTUPCLRCLN 50 ptile','LWTUPCLRCLN 70 ptile','LWTUPCLRCLN 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='LWTUPCLRCLN';
ylabel('Monthly LW clear sky/no aerosol upwelling toa flux -W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('LWTUPCLRCLN','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' LWTUPCLRCLN Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of the long wave clear sky/no aerosol upwelling flux at the toa.';
