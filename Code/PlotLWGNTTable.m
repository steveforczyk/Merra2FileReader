function PlotLWGNTTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function will plot the LWGNT Table Values
...
plot(LWGNTTT.Time,LWGNTTT.LWGNT10,'b',LWGNTTT.Time,LWGNTTT.LWGNT50,'g',...
    LWGNTTT.Time,LWGNTTT.LWGNT70,'c',LWGNTTT.Time,LWGNTTT.LWGNT90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('LWGNT 10 ptile','LWGNT 50 ptile','LWGNT 70 ptile','LWGNT 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='LWGEM';
ylabel('Monthly Longwave Surface Net Downward  Flux -W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('LWGNT','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' LWGNT Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of the long wave surface net downwards flux.';
