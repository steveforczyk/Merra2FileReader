function PlotLWGEMTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function will plot the LWGEM Table Values
...
plot(LWGEMTT.Time,LWGEMTT.LWGEM10,'b',LWGEMTT.Time,LWGEMTT.LWGEM50,'g',...
    LWGEMTT.Time,LWGEMTT.LWGEM70,'c',LWGEMTT.Time,LWGEMTT.LWGEM90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('LWGEM 10 ptile','LWGEM 50 ptile','LWGEM 70 ptile','LWGEM 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='LWGEM';
ylabel('Monthly Longwave Surface Emitted Flux -W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('LWGEM','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' LWGEM Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of the long wave surface flux emission.';
