function  DisplayMerra2SeaIceFraction(ikind,titlestr)
...
if(ikind==1)
    SeaIceT=FRSEAICES.values;
    SeaIce=SeaIceT(:,:,iTimeSlice);
    vmax=FRSEAICES.vmax;
    vmin=FRSEAICES.vmin;
    minval=0;
    maxval=1.2;
...
%% Plot the Sea Ice Fraction so remove bad values
[nrows,ncols]=size(SeaIce);
numpts=nrows*ncols;
nanval=FillVal;
[SeaIceAdj,numNan] = RemoveNaNValues(SeaIce,nanval);
% Define a new array where all values less than 0 are set to 0 the
%values above 1 are set to 1 all others are untouched
[ibad,jbad]=find((SeaIce<vmin) | (SeaIce>vmax));
numbad=length(ibad);
ilow=0;
ihigh=0;
if(numbad>0)
    for jj=1:numbad
        indx=ibad(jj,1);
        indy=jbad(jj,1);
        nowval=SeaIceAdj(indx,indy);
        if(nowval<0)
            SeaIceAdj(indx,indy)=0;
            ilow=ilow+1;
        elseif(nowval>1)
            SeaIceAdj(indx,indy)=0;
            ihigh=ihigh+1;
...
SeaIceAdjwMean=SeaIceAdj;
startmean=mean(mean(SeaIceAdj));
% Calculate some stats
SeaIce1D=reshape(SeaIceAdj,nrows*ncols,1);
SeaIce1DS=sort(SeaIce1D);
numpts1=length(SeaIce1DS);
numpts2=numpts1-numNan;
SeaIce1DwMean=reshape(SeaIceAdjwMean,nrows*ncols,1);
SeaIce1DSwMean=sort(SeaIce1DwMean);
num01=floor(.01*numpts2);
num25=floor(.25*numpts2);
num50=floor(.50*numpts2);
num75=floor(.75*numpts2);
num99=floor(.99*numpts2);
val01=SeaIce1DS(num01,1);
val25=SeaIce1DS(num25,1);
val50=SeaIce1DS(num50,1);
val75=SeaIce1DS(num75,1);
val99=SeaIce1DS(num99,1);
val01wMean=SeaIce1DSwMean(num01,1);
val25wMean=SeaIce1DSwMean(num25,1);
val50wMean=SeaIce1DSwMean(num50,1);
val75wMean=SeaIce1DSwMean(num75,1);
val99wMean=SeaIce1DSwMean(num99,1);
headerstr=strcat('Basic Stats wMeans follow for-',desc,'-Data');
fprintf(fid,'%s\n',headerstr);
ptc1str=strcat('01 % SeaIce Value=',num2str(val01wMean,6));
fprintf(fid,'%s\n',ptc1str);
ptc25str=strcat('25 % SeaIce Value=',num2str(val25wMean,6));
fprintf(fid,'%s\n',ptc25str);
ptc50str=strcat('50 % SeaIce Value=',num2str(val50wMean,6));
fprintf(fid,'%s\n',ptc50str);
ptc75str=strcat('75 % SeaIce Value=',num2str(val75wMean,6));
fprintf(fid,'%s\n',ptc75str);
ptc99str=strcat('99 % SeaIce Vallue=',num2str(val99wMean,6));
fprintf(fid,'%s\n',ptc99str);
numbadstr=strcat('NumBadFrac=',num2str(numbadfrac,6),'-Array Mean Value=',num2str(startmean,6));
fprintf(fid,'%s\n',numbadstr);
fprintf(fid,'%s\n',' End Stats for SeaIce Data');
...
geoshow(SeaIceAdj',Rpix,'DisplayType','surface');
%zlimits=[minval maxval];
%demcmap(zlimits);
