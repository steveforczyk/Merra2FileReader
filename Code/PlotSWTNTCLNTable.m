function PlotSWTNTCLNTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
...
plot(SWTNTCLNTT.Time,SWTNTCLNTT.SWTNTCLN10,'b',SWTNTCLNTT.Time,SWTNTCLNTT.SWTNTCLN50,'g',...
    SWTNTCLNTT.Time,SWTNTCLNTT.SWTNTCLN70,'c',SWTNTCLNTT.Time,SWTNTCLNTT.SWTNTCLN90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('SWTNTCLN 10 ptile','SWTNTCLN 50 ptile','SWTNTCLN 70 ptile','SWTNTCLN 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='SWTNTCLN';
ylabel('Monthly Toa Net DownwardsShortWave No Aero Flux Average-W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('SWTNTCLN','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' SWTNTCLN Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of TOA Net Downward SW No Aero flux at all levels.';
