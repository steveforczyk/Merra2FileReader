function PlotLWGNTCLRCLNTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function will plot the LWGNTCLRCLN Table Values
...
plot(LWGNTCLRCLNTT.Time,LWGNTCLRCLNTT.LWGNTCLRCLN10,'b',LWGNTCLRCLNTT.Time,LWGNTCLRCLNTT.LWGNTCLRCLN50,'g',...
    LWGNTCLRCLNTT.Time,LWGNTCLRCLNTT.LWGNTCLRCLN70,'c',LWGNTCLRCLNTT.Time,LWGNTCLRCLNTT.LWGNTCLRCLN90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('LWGNTCLRCLN 10 ptile','LWGNTCLRCLN 50 ptile','LWGNTCLRCLN 70 ptile','LWGNTCLRCLN 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='LWGNTCLRCLN';
ylabel('Monthly Longwave Surface Net Downward Clear Sky Flux/NoAero -W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('LWGNTCLRCLN','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' LWGNTCLRCLN Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of the long wave surface net downwards flux for clear sky and no aerosol.';
