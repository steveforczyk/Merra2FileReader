function  DisplayMerra2EMISS(titlestr)
% Display the surface emissivity
% Written By: Stephen Forczyk
% Created: Aug 10,2022
% Revised: Aug 16,2022 added capability to subtract out mean from plot
...
global EMISS LWGABS LWGABCLRS TSS EMISS;
...
fprintf(fid,'%s\n','------- Plot The Surface Emissivity ------');
EMIS=EMISS.values;
desc='Surface Emissvity';
%% Plot the Surface Emissivity so remove bad values
[nrows,ncols]=size(EMIS);
numpts=nrows*ncols;
EMISAdj=EMIS;
EMISAdjwMean=EMIS;
...
startmean=mean(mean(EMISAdj));
iSubMean1=0;
isub=0;
if(iSubMean*iSubMean1>0)
    EMISAdj=EMISAdj-startmean;
    isub=1;
else
...
EMIS1D=reshape(EMISAdj,nrows*ncols,1);
EMIS1DS=sort(EMIS1D);
EMISAdj1DwMean=reshape(EMISAdjwMean,nrows*ncols,1);
EMISAdj1DSwMean=sort(EMISAdj1DwMean);
num01=floor(.01*numpts);
num25=floor(.25*numpts);
num50=floor(.50*numpts);
num75=floor(.75*numpts);
num99=floor(.99*numpts);
val01=EMIS1DS(num01,1);
val25=EMIS1DS(num25,1);
val50=EMIS1DS(num50,1);
val75=EMIS1DS(num75,1);
val99=EMIS1DS(num99,1);
val01wMean=EMISAdj1DSwMean(num01,1);
val25wMean=EMISAdj1DSwMean(num25,1);
val50wMean=EMISAdj1DSwMean(num50,1);
val75wMean=EMISAdj1DSwMean(num75,1);
val99wMean=EMISAdj1DSwMean(num99,1);
fprintf(fid,'%s\n',' Basic Stats follow for Surface Emissivity with Mean Data');
ptc1str=strcat('01 % Ptile Emissivity=',num2str(val01wMean,6));
fprintf(fid,'%s\n',ptc1str);
ptc25str=strcat('25 % Ptile Emissivity=',num2str(val25wMean,6));
fprintf(fid,'%s\n',ptc25str);
ptc50str=strcat('50 % Ptile Emissivity=',num2str(val50wMean,6));
fprintf(fid,'%s\n',ptc50str);
ptc75str=strcat('75 % Ptile Emissivity=',num2str(val75wMean,6));
fprintf(fid,'%s\n',ptc75str);
ptc99str=strcat('99 % Ptile Emissivity=',num2str(val99wMean,6));
fprintf(fid,'%s\n',ptc99str);
...
geoshow(EMISAdj',Rpix,'DisplayType','surface');
zlimits=[minval maxval];
%demcmap(zlimits);
%demcmap('inc',EMISAdj',dinc);
...
tx2=.10;
ty2=.14;
txtstr2=strcat('1 ptile Emissivity=',num2str(val01,6),'//-50 ptile =',num2str(val50,6),...
    '//-99 ptile=',num2str(val99,6));
...
pause(chart_time);
fprintf(fid,'%s\n','------- Finished Plotting Surface Emissity ------');
close('all');
end
