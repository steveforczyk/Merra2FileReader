function PlotAlebedoVisBeamTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function is plotting the Surface Visible Beam Table Values
...
global AlbvisdfTable  AlbvisdfTT AlbvisdrTable  AlbvisdrTT;
...
plot(AlbvisdrTT.Time,AlbvisdrTT.Albvisdr10,'b',AlbvisdrTT.Time,AlbvisdrTT.Albvisdr50,'g',...
    AlbvisdrTT.Time,AlbvisdrTT.Albvisdr70,'c',AlbvisdrTT.Time,AlbvisdrTT.Albvisdr90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('Albvisdr 10 ptile','Albvisdr 50 ptile','Albvisdr 70 ptile','Albvisdr 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='Albvisdr';
ylabel('Monthly Albvisdr Average','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('ALBVISDR','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('Visible Albedo Beam Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of the surface visible beam albedo.';
