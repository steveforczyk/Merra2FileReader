function PlotLWGABTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function is the first attempt at plotting the LWGAB Table Values
%  Later this may be rewritten to handle multiple types of tables
%
% Written By: Stephen Forczyk
% Created: April 11,2023
% Revised: April 16,2023 added output to PDF Report
...
plot(LWGABTT.Time,LWGABTT.LWGAB10,'b',LWGABTT.Time,LWGABTT.LWGAB50,'g',...
    LWGABTT.Time,LWGABTT.LWGAB70,'c',LWGABTT.Time,LWGABTT.LWGAB90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('LWGAB 10 ptile','LWGAB 50 ptile','LWGAB 70 ptile','LWGAB 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='LWGAB';
ylabel('Monthly Longwave Surface Absorption Average-W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('LWGAB','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' LWGAB Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of the surface absorbed LW radiation.';
