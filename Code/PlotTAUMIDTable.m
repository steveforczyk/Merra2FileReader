function PlotTAUMIDTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function is plots the TAUMID Table Values
% 
%
...
plot(TAUMIDTT.Time,TAUMIDTT.TAUMID10,'b',TAUMIDTT.Time,TAUMIDTT.TAUMID50,'g',...
    TAUMIDTT.Time,TAUMIDTT.TAUMID70,'c',TAUMIDTT.Time,TAUMIDTT.TAUMID90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('TAUMID 10 ptile','TAUMID 50 ptile','TAUMID 70 ptile','TAUMID 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='TAUMID';
ylabel('Monthly Mid Cloud Optical Thickness','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('TAUMID','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' TAUMID Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of mid cloud optical thickness.';
