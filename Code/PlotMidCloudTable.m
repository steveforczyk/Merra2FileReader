function PlotMidCloudTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function is designed to plot the Mid Cloud Fraction Table Values
%  
%
...
plot(CLDMIDTT.Time,CLDMIDTT.CLDMID10,'b',CLDMIDTT.Time,CLDMIDTT.CLDMID50,'g',...
    CLDMIDTT.Time,CLDMIDTT.CLDMID70,'c',CLDMIDTT.Time,CLDMIDTT.CLDMID90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('MID CLD Frac 10 ptile','MID CLD Frac 50 ptile','MID CLD Frac 70 ptile','MID CLD Frac 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabel('Monthly MID Cloud Frac Average','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('MidCloud','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('Mid Cloud Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average fractional cloud cover at Mid altitude (about 5 km).';
