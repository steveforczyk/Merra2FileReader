function PlotTAUTOTTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function is plots the TAUTOT Table Values
% 
%
...
global TAULOWTabel TAULOWTT TAUTOTTabel TAUTOTTT;
...
plot(TAUTOTTT.Time,TAUTOTTT.TAUTOT10,'b',TAUTOTTT.Time,TAUTOTTT.TAUTOT50,'g',...
    TAUTOTTT.Time,TAUTOTTT.TAUTOT70,'c',TAUTOTTT.Time,TAUTOTTT.TAUTOT90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('TAUTOT 10 ptile','TAUTOT 50 ptile','TAUTOT 70 ptile','TAUTOT 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='TAUTOT';
ylabel('Monthly TOT Cloud Optical Thickness','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('TAUTOT','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' TAUTOT Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of total cloud optical thickness.';
