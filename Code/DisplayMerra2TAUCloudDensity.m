function  DisplayMerra2TAUCloudDensity(ikind,titlestr)
% Display the cloud optical thickness for high clouds expressed as 
% thickness per sq km
% Written By: Stephen Forczyk
% Created: Oct 25,2022
% Revised: Oct 26,2022 added RasterArea variable to get density plot
% Classification: Unclassified

global LonS LatS TimeS ;
global YearMonthDayStr1 YearMonthStr Year Month
global TAUHGHS TAULOWS TAUMIDS TAUTOTS TAUDensity minTauValue TauArea;
global LatSpacing LonSpacing RasterAreas AreaOfEarth EarthAreaFraction;
...
TAUDensity=zeros(numlons,numlats);
% Now divide the TAUVAL array elements by the area of that latitude
for j=1:numlats
    nowArea=RasterAreas(j,1);
    for i=1:numlons
        if(nowArea<1)
            nowArea=1;
        end
        TAUDensity(i,j)=TAUVAL(i,j)/nowArea;
    end
end
...
% Get the area of all rasters with TAUVAL>=minTauValue
% Define a new array where all values less than 0 are set to 0 the
% others are untouched
[ibad,jbad]=find(TAUVAL<minTauValue);
numbad=length(ibad);
ilow=0;
[ibig,jbig]=find(TAUVAL>=minTauValue);
numbig=length(ibig);
TauArea=0;
AreaOfEarth=510.1E6; % In square km
if(numbig>0)
    for nn=1:numbig
        inow=ibig(nn,1);
        jnow=jbig(nn,1);
        nowArea=RasterAreas(jnow,1); 
        TauArea=TauArea+nowArea;
    end
    EarthAreaFraction=TauArea/AreaOfEarth;
...
earthareastr=strcat('Area Of High COT=',num2str(TauArea,6),'-EarthAreaFrac=',num2str(EarthAreaFraction,6));
fprintf(fid,'%s\n',earthareastr);
if(numbad>0)
    for jj=1:numbad
        indx=ibad(jj,1);
...
TAUDen1D=reshape(TAUDensity,nrows*ncols,1);
TAUDen1DS=sort(TAUDen1D);
% Remove the NaNs from the sort array for statistical purposes
inan=0;
igood=0;
for n=1:numpts
    TF1=isnan(TAUDen1DS(n,1));
    if(TF1==1)
        inan=inan+1;
    else
        igood=n;
...
numpts2=igood;
% TAUVALAdj1DwMean=reshape(TauValAdjwMean,nrows*ncols,1);
% TAUVALAdj1DSwMean=sort(TAUVALAdj1DwMean);
num01=floor(.01*numpts2);
num25=floor(.25*numpts2);
num50=floor(.50*numpts2);
num75=floor(.75*numpts2);
num99=floor(.99*numpts2);
val01=TAUDen1DS(num01,1);
val25=TAUDen1DS(num25,1);
val50=TAUDen1DS(num50,1);
val75=TAUDen1DS(num75,1);
val99=TAUDen1DS(num99,1);
fprintf(fid,'%s\n',' Basic Stats follow for Cloud Optical Density Data');
ptc1str= strcat('01 % Ptile Cloud Optical Density=',num2str(val01,6));
fprintf(fid,'%s\n',ptc1str);
ptc25str=strcat('25 % Ptile Cloud Optical Density=',num2str(val25,6));
fprintf(fid,'%s\n',ptc25str);
ptc50str=strcat('50 % Ptile Cloud Optical Density=',num2str(val50,6));
fprintf(fid,'%s\n',ptc50str);
ptc75str=strcat('75 % Ptile Cloud Optical Density=',num2str(val75,6));
fprintf(fid,'%s\n',ptc75str);
ptc99str=strcat('99 % Ptile Cloud Optical Density=',num2str(val99,6));
fprintf(fid,'%s\n',ptc99str);
fprintf(fid,'%s\n',' End Stats follow for Cloud Optical Density Data');
...
geoshow(TAUDensity',Rpix,'DisplayType','surface');
c1=colormap('jet');
cmapland(1:181,:)=c1(1:181,:);
demcmap(TAUDensity',181,[],cmapland);
hc=colorbar;
...
tx2=.10;
ty2=.14;
txtstr2=strcat('1 ptile Cloud Optical Density =',num2str(val01,6),'-50 ptile =',num2str(val50,6),...
    '-75 ptile=',num2str(val75,6),'-99 ptile=',num2str(val99,6));
txt2=text(tx2,ty2,txtstr2,'FontWeight','bold','FontSize',10);
tx3=.10;
ty3=.10;
txtstr3=strcat('COT Area Above Threshold=',num2str(TauArea,6),'-on-',num2str(numbig,6),...
    '-pixels or-',num2str(EarthAreaFraction,6),'-Fraction Of Earth Surface');
...
pause(chart_time);
fprintf(fid,'%s\n','------- Finished Plotting Cloud Density High Clouds ------');
close('all');
end
