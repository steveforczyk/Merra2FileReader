function [Stats,inArrayAdj,fraclow,frachigh,fracNaN] = GetDistributionStatsRev6(inVar,lowcutoff,highcutoff)
% This function will return some crude statistics of the 3 D array
% the format of the array this routine is mean to deal with is 576 x 361 x
% 24  . The 3D aspect is time which is reported in one hour increments,
% This differs from the Rev 5 version in that the 24 time segments are
% integrated together
...
% Created: Jan 28,2024
% Revised: Feb 1,2024 added logic to sum 24 segments each 1 hr long and
% thus create an average daily rate
...
FillVal=inVar.FillValue;
inArrayAll=inVar.values;
[nrows,ncols,nslices]=size(inArrayAll);
ntotal=nrows*ncols;
numpts=ntotal;
inArrayAdj1=zeros(nrows,ncols);
nanval=FillVal;
for k=1:24
    inArray=inArrayAll(:,:,k);
    inArray=inArray*iScale;
    [inArrayAdj2,numNan] = RemoveNaNValues(inArray,nanval);
    inArrayAdj=inArrayAdj1+inArrayAdj2;
    inArrayAdj1=inArrayAdj;
end
[inArrayAdj2,numNan] = RemoveNaNValues(inArrayAdj,nanval);
fracNaN=numNan/ntotal;
%InArrayAdj=inArrayAdj2/24;% divide by 24 for the hourly sums
InArrayAdj=inArrayAdj2/24;% sum up to get daily total
