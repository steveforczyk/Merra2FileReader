function PlotLWGABCLRTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function will plot the LWGABCLR Table Values
...
global LWGABCLRTable LWGABCLRTT;
...
plot(LWGABCLRTT.Time,LWGABCLRTT.LWGABCLR10,'b',LWGABCLRTT.Time,LWGABCLRTT.LWGABCLR50,'g',...
    LWGABCLRTT.Time,LWGABCLRTT.LWGABCLR70,'c',LWGABCLRTT.Time,LWGABCLRTT.LWGABCLR90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('LWGABCLR 10 ptile','LWGABCLR 50 ptile','LWGABCLR 70 ptile','LWGABCLR 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='LWGABCLR';
ylabel('Monthly Longwave Surface Absorption Clear Sky Average-W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('LWGABCLR','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' LWGABCLR Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of the surface absorbed LW radiation for clear sky.';
