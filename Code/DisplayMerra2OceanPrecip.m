function  DisplayMerra2OceanPrecip(Stats,PrecipAdj,fraclow,frachigh,fracNaN,ikind,titlestr)
% Display the downward net  LW flux for dataset 04
% This code is intended to display the snowfall/rainfall amounts of
% precip over ocea areas. The data can be plotted as a rate or over a 24
% hour period
% 
...
% The precip either as a rate or a total can not be less than zero to set
% very low and negative values to NaNs. This will help the plots to look
% better
[nrows,ncols]=size(PrecipAdj);
ntotal=nrows*ncols;
PrecipAdj2=PrecipAdj;
iAddNaN=0;
for ii=1:nrows
    for jj=1:ncols
        nowVal=PrecipAdj(ii,jj);
        a1=isnan(nowVal);
        if(a1==0)
            if(nowVal<.001)
               PrecipAdj(ii,jj)=NaN; 
...
%% Plot the surface Precip on the map
geoshow(PrecipAdj2',Rpix,'DisplayType','surface');
