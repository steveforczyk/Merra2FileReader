function ReadDailyCOTDataTo1Day(nc_filename)
...
% This version will read many files and convert to 1 day averages
% Written By: Stephen Forczyk
% Created: Mar 22,2023
% Revised: Mar 24,2023 averaged surface pressure over 1 day in this routine
...
global TauCliSel TauClwSel SurfacePressure SP SurfPressure1D;
...
CloudGrab=zeros(nlon,nlat);
CloudHold=zeros(nlon,nlat);
...
%                CloudHold(n1,n2)=CloudHold(n1,n2)+CloudGrab(n1,n2);
            end
        end
...
TauCli200hPA1Day=TauCliSel1Day(:,:,1);
TauCli300hPA1Day=TauCliSel1Day(:,:,2);
TauCli400hPA1Day=TauCliSel1Day(:,:,3);
ab=2;
% Pull out the surface pressures
SP= PsS.values;
[k1,k2,ntimes]=size(SP);
SurfPressure1D=zeros(k1,k2);
for ii=1:k1
    for jj=1:k2
        for kk=1:ntimes
            SurfPressure1D(ii,jj)=SurfPressure1D(ii,jj)+SP(ii,jj,kk);
        end
    end
end
SurfPressure1D=SurfPressure1D/8;
ab=2;
%% This section will save the data from the 3 hr time segments
isavefiles=2;
MatFileName=strcat('COT-3Hr-Rev1-',Merra2ShortFileName,'.mat');
...
if(isavefiles>0)
    ...
    varstr2=' TauCliSel TauClwSel PressureLevelUsed timeS SP';
    varstr3=' CloudSel CfcuSel TauCli200hPA1Day TauCli300hPA1Day TauCli400hPA1Day';
    varstr4=' CloudSel1Day CfcuSel1Day TauCliSel1Day TauClwSel1Day SurfPresure1D';
...
%% PLot selected variables that were average into 1 Day time segments
% Start with the cloud optical thickness
ikind=1;
ipress=3;
itype=3;
varname='TAUCLI';
