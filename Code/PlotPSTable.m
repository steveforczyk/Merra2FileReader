function PlotPSTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
...
plot(PSTT.Time,PSTT.PS25,'b',PSTT.Time,PSTT.PS50,'g',...
    PSTT.Time,PSTT.PS75,'k',PSTT.Time,PSTT.PS90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('PS 25 ptile','PS 50 ptile','PS 75 ptile','PS 90 ptile');
ht=title(titlestr);
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('PS','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' PS Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is the instantaneous surface pressure in psi.';
    parastr3='The expected data range for this dataset is from 0 to 20 psi.';
