function PlotSWGNCLNTTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function will plot the SWGNTCLN Table Values
%
...
global SWGNTCLNTable SWGNTCLNTT;
...
plot(SWGNTCLNTT.Time,SWGNTCLNTT.SWGNTCLN10,'b',SWGNTCLNTT.Time,SWGNTCLNTT.SWGNTCLN50,'g',...
    SWGNTCLNTT.Time,SWGNTCLNTT.SWGNTCLN70,'c',SWGNTCLNTT.Time,SWGNTCLNTT.SWGNTCLN90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('SWGNTCLN 10 ptile','SWGNTCLN 50 ptile','SWGNTCLN 70 ptile','SWGNTCLN 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='SWGNTCLN';
ylabel('Monthly surface net downwards SW flux No Aerosol  -W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('SWGNTCLN','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' SWGNTCLN Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of the surface net downwards SW flux assuming no aerosol.';
