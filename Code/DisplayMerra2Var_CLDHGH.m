function  DisplayMerra2Var_CLDHGH(ikind,titlestr)
% Display the variance of the cloud fraction
...
iSubMean=0;
if(ikind==1)
    fprintf(fid,'%s\n','------- Plot The High Cloud Cover Fraction Variance ------');
    VarQuant=VarCLDHGHS.values;
    desc='HighCloud-Cover-Frac-Var';
    units='cloud cover frac';
    iSubMean=1;
elseif(ikind==2)
    fprintf(fid,'%s\n','------- Plot The Low Cloud Cover Fraction Variance------');
    VarQuant=VarCLDLOWS.values;
    desc='LowCloud-Cover-Frac-Var';
    units='cloud cover frac';
    iSubMean=0;
elseif(ikind==3)
    fprintf(fid,'%s\n','------- Plot The Mid Cloud Cover Fraction Variance------');
    VarQuant=VarCLDMIDS.values;
    desc='MidCloud-Cover-Frac-Var';
    units='cloud cover frac';
    iSubMean=1;
elseif(ikind==4)
    fprintf(fid,'%s\n','------- Plot The Total Cloud Cover Fraction Variance------');
    VarQuant=VarCLDTOTS.values;
    desc='TotCloud-Cover-Frac-Var';
    units='cloud cover frac';
    iSubMean=1;
elseif(ikind==5)
...
maxval=1.5*val90;
maxval2=1.2*maxval;
if((ikind==1) || (ikind==2) || (ikind==3) || (ikind==4))
    if(iSubMean==0)
        minval=0;
        maxval=2*val75;
        maxval2=maxval+.1;
        ...
        maxval=val99;
        maxval2=maxval+.05;
        units=strcat('cloud cover frac-',num2str(subvalue,6));
    end
end
% if(ikind==2)
%     minval=0;
...
if(iCityPlot>0)
    for k=1:maxCities
        ...
        ab=1;
        plot3m(EuropeLat,EuropeLon,maxval2,'b');
    end
end
hold off
