function  DisplayMerra2TSS(titlestr)
% Display the surface skin temp
% Written By: Stephen Forczyk
% Created: Aug  9,2022
% Revised: Aug 17,2022 add stats data to output file
...
fprintf(fid,'%s\n','------- Plot The Surface Skin Temp ------');
TSkin=TSS.values;
%% Plot the Surface Skin Temp so remove bad values
[nrows,ncols]=size(TSkin);
numpts=nrows*ncols;
TSkinAdj=TSkin;
% Define a new array where all values less than 0 are set to 0 the
% others are untouched
[ibad,jbad]=find(TSkin<0);
numbad=length(ibad);
ilow=0;
if(numbad>0)
    for jj=1:numbad
        indx=ibad(jj,1);
        indy=jbad(jj,1);
        nowval=TSkin(indx,indy);
        if(nowval<0)
            TSkinAdj(indx,indy)=0;
            ilow=ilow+1;
        end
...
TSkin1D=reshape(TSkinAdj,nrows*ncols,1);
TSkin1DS=sort(TSkin1D);
num01=floor(.01*numpts);
num25=floor(.25*numpts);
num50=floor(.50*numpts);
num75=floor(.75*numpts);
num99=floor(.99*numpts);
val01=TSkin1DS(num01,1);
val25=TSkin1DS(num25,1);
val50=TSkin1DS(num50,1);
val75=TSkin1DS(num75,1);
val99=TSkin1DS(num99,1);
fprintf(fid,'%s\n',' Basic Stats follow for Surface Skin Temp with Mean Data');
ptc1str=strcat('01 % Ptile Surface Temp Deg K =',num2str(val01,6));
fprintf(fid,'%s\n',ptc1str);
ptc25str=strcat('25 % Ptile Surface Temp Deg K=',num2str(val25,6));
fprintf(fid,'%s\n',ptc25str);
ptc50str=strcat('50 % Ptile Surface Temp Deg K=',num2str(val50,6));
fprintf(fid,'%s\n',ptc50str);
ptc75str=strcat('75 % Ptile Surface Temp Deg K=',num2str(val75,6));
fprintf(fid,'%s\n',ptc75str);
ptc99str=strcat('99 % Ptile Surface Temp Deg K=',num2str(val99,6));
fprintf(fid,'%s\n',ptc99str);
fprintf(fid,'%s\n',' End Stats For Surface Slin Temp Data');
minval=200;
maxval=350;
maxval2=maxval+5;
...
geoshow(TSkinAdj',Rpix,'DisplayType','surface');
zlimits=[minval maxval];
%demcmap(zlimits);
demcmap('inc',[maxval minval],incsize);
hc=colorbar;
ylabel(hc,'Temp-Deg K','FontWeight','bold');
tightmap
hold on
...
tx2=.10;
ty2=.14;
txtstr2=strcat('1 ptile TSkin Temp=',num2str(val01,6),'-50 ptile =',num2str(val50,6),...
    '-99 ptile=',num2str(val99,6),'-DegK');
...
pause(chart_time);
fprintf(fid,'%s\n','------- Finished Plotting The Surface Skin Temp ------');
close('all');
end
