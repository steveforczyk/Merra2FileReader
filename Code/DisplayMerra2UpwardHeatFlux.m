function  DisplayMerra2UpwardHeatFlux(ikind,titlestr)
% Display the upward het flux for dataset 04
...
if(ikind==1)
    HFluxT=HFLUXICES.values;
    HFlux=HFluxT(:,:,iTimeSlice);
    vmax=HFLUXICES.vmax;
    vmin=HFLUXICES.vmin;
    minval=-200;
    maxval=500;
    FillVal=HFLUXICES.FillValue;
    desc='Sea Ice Upward HeatFlux';
    unitstr='W/m2';
elseif(ikind==2)
    HFluxT=HFLUXWTRS.values;
    vmax=HFLUXWTRS.vmax;
    vmin=HFLUXWTRS.vmin;
    minval=-200;
    maxval=1000;
    HFlux=HFluxT(:,:,iTimeSlice);
    FillVal=HFLUXWTRS.FillValue;
    desc='Open Water Upward Het flux';
    unitstr='W/m2';
end
%% Plot the Upward Heat Flux so remove bad values
[nrows,ncols]=size(HFlux);
numpts=nrows*ncols;
nanval=FillVal;
[HFluxAdj,numNan] = RemoveNaNValues(HFlux,nanval);
% Define a new array where all values less than 0 are set to 0 the
%values above 1 are set to 1 all others are untouched
[ibad,jbad]=find((HFlux<vmin) | (HFlux>vmax));
numbad=length(ibad);
ilow=0;
ihigh=0;
if(numbad>0)
    for jj=1:numbad
        indx=ibad(jj,1);
        indy=jbad(jj,1);
        nowval=HFluxAdj(indx,indy);
        if(nowval<0)
            HFluxAdj(indx,indy)=0;
            ilow=ilow+1;
        elseif(nowval>1)
            HFluxAdj(indx,indy)=0;
            ihigh=ihigh+1;
...
HFluxAdjwMean=HFluxAdj;
startmean=mean(mean(HFluxAdj));
% Calculate some stats
HFlux1D=reshape(HFluxAdj,nrows*ncols,1);
HFlux1DS=sort(HFlux1D);
numpts1=length(HFlux1DS);
numpts2=numpts1-numNan;
HFlux1DwMean=reshape(HFluxAdjwMean,nrows*ncols,1);
HFlux1DSwMean=sort(HFlux1DwMean);
num01=floor(.01*numpts2);
num25=floor(.25*numpts2);
num50=floor(.50*numpts2);
num75=floor(.75*numpts2);
num99=floor(.99*numpts2);
val01=HFlux1DS(num01,1);
val25=HFlux1DS(num25,1);
val50=HFlux1DS(num50,1);
val75=HFlux1DS(num75,1);
val99=HFlux1DS(num99,1);
val01wMean=HFlux1DSwMean(num01,1);
val25wMean=HFlux1DSwMean(num25,1);
val50wMean=HFlux1DSwMean(num50,1);
val75wMean=HFlux1DSwMean(num75,1);
val99wMean=HFlux1DSwMean(num99,1);
headerstr=strcat('Basic Stats wMeans follow for-',desc,'-Data');
fprintf(fid,'%s\n',headerstr);
ptc1str=strcat('01 % HFlux Value=',num2str(val01wMean,6));
fprintf(fid,'%s\n',ptc1str);
ptc25str=strcat('25 % HFlux Value=',num2str(val25wMean,6));
fprintf(fid,'%s\n',ptc25str);
ptc50str=strcat('50 % HFlux Value=',num2str(val50wMean,6));
fprintf(fid,'%s\n',ptc50str);
ptc75str=strcat('75 % HFlux Value=',num2str(val75wMean,6));
fprintf(fid,'%s\n',ptc75str);
ptc99str=strcat('99 % HFlux Vallue=',num2str(val99wMean,6));
fprintf(fid,'%s\n',ptc99str);
...
geoshow(HFluxAdj',Rpix,'DisplayType','surface');
%zlimits=[minval maxval];
%demcmap(zlimits);
