function PlotSWGNTTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function will plot the SWGNT Table Values
...
plot(SWGNTTT.Time,SWGNTTT.SWGNT10,'b',SWGNTTT.Time,SWGNTTT.SWGNT50,'g',...
    SWGNTTT.Time,SWGNTTT.SWGNT70,'c',SWGNTTT.Time,SWGNTTT.SWGNT90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('SWGNT 10 ptile','SWGNT 50 ptile','SWGNT 70 ptile','SWGNT 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='SWGNT';
ylabel('Monthly surface net downwards SW flux  -W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('SWGNT','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' SWGNT Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of the surface net downwards SW flux.';
