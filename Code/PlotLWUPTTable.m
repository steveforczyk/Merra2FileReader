function PlotLWUPTTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function will plot the LWUPT Table Values
...
plot(LWTUPTT.Time,LWTUPTT.LWTUP10,'b',LWTUPTT.Time,LWTUPTT.LWTUP50,'g',...
    LWTUPTT.Time,LWTUPTT.LWTUP70,'c',LWTUPTT.Time,LWTUPTT.LWTUP90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('LWTUP 10 ptile','LWTUP 50 ptile','LWTUP 70 ptile','LWTUP 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='LWTUP';
ylabel('Monthly LW upwelling toa flux -W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('LWTUP','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' LWTUO Map For File-',Merra2ShortFileName);
