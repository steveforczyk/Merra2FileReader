function PlotSWGNCLRCLNTTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
...
global SWGNTCLRCLNTable SWGNTCLRCLNTT;
...
plot(SWGNTCLRCLNTT.Time,SWGNTCLRCLNTT.SWGNTCLRCLN10,'b',SWGNTCLRCLNTT.Time,SWGNTCLRCLNTT.SWGNTCLRCLN50,'g',...
    SWGNTCLRCLNTT.Time,SWGNTCLRCLNTT.SWGNTCLRCLN70,'c',SWGNTCLRCLNTT.Time,SWGNTCLRCLNTT.SWGNTCLRCLN90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('SWGNTCLRCLN 10 ptile','SWGNTCLRCLN 50 ptile','SWGNTCLRCLN 70 ptile','SWGNTCLRCLN 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='SWGNTCLRCLN';
ylabel('Monthly surface net downwards SW flux Clear Sky No Aero -W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('SWGNTCLRCLN','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' SWGNTCLRCLN Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of the surface net downwards SW flux assuming clear sky and no aerosol.';
