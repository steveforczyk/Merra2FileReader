function PlotSWTNTTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
...
plot(SWTNTTT.Time,SWTNTTT.SWTNT10,'b',SWTNTTT.Time,SWTNTTT.SWTNT50,'g',...
    SWTNTTT.Time,SWTNTTT.SWTNT70,'c',SWTNTTT.Time,SWTNTTT.SWTNT90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('SWTNT 10 ptile','SWTNT 50 ptile','SWTNT 70 ptile','SWTNT 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='SWTNT';
ylabel('Monthly Toa Net DownwardsShortWave Flux Average-W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('SWTNT','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' SWTNT Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of TOA Net Downward SW flux at all levels.';
