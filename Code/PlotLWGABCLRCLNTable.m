function PlotLWGABCLRCLNTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function will plot the LWGABCLRCLN Table Values
...
plot(LWGABCLRCLNTT.Time,LWGABCLRCLNTT.LWGABCLRCLN10,'b',LWGABCLRCLNTT.Time,LWGABCLRCLNTT.LWGABCLRCLN50,'g',...
    LWGABCLRCLNTT.Time,LWGABCLRCLNTT.LWGABCLRCLN70,'c',LWGABCLRCLNTT.Time,LWGABCLRCLNTT.LWGABCLRCLN90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('LWGABCLRCLN 10 ptile','LWGABCLRCLN 50 ptile','LWGABCLRCLN 70 ptile','LWGABCLRCLN 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='LWGABCLRCLN';
ylabel('Monthly Longwave Surface Absorption Clear Sky Average No Aerosol-W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('LWGABCLRCLN','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' LWGABCLRCLN Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of the surface absorbed LW radiation for clear sky with no aerosol.';
