function ReadDataset07(nowFile,nowpath)
% Modified: This function will read in the the Merra-2 data set called 07
% which is hourly average data 4 times a day for up to 42 pressure levels
...
global HS O3S PSS QVS SLPS TS US VS;
global iLVLS;
...
HS=struct('values',[],'long_name',[],'units',[],'FillValue',[],...
...
for i = 0:numvars-1
    ...
    flag = 0;
    for j = 0:numatts - 1
       a10=strcmp(varname,'H');
       a20=strcmp(varname,'O3');
...
%        a240=strcmp(varname,'SWGNTWTR');
       if (a10==1)
            ...
            if strmatch('add_offset',attname1)
                offset = attname2;
                HS.add_offset=attname2;
            end
            if strmatch('scale_factor',attname1)
                scale = attname2;
                HS.scale_factor=attname2;
        ...
        elseif (a20==1)
            ...
            if strmatch('add_offset',attname1)
                offset = attname2;
               O3S.add_offset=attname2;
            end
            if strmatch('scale_factor',attname1)
                scale = attname2;
                O3S.scale_factor=attname2;
                flag = 0;
            end 
