function PlotSWDTNTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function is the first attempt at plotting the SWDTN Table Values
% 
%
% Written By: Stephen Forczyk
% Created: April 13,2023
% Revised: April logic to include this chart in the PDF report
...
plot(SWTDNTT.Time,SWTDNTT.SWTDN10,'b',SWTDNTT.Time,SWTDNTT.SWTDN50,'g',...
    SWTDNTT.Time,SWTDNTT.SWTDN70,'c',SWTDNTT.Time,SWTDNTT.SWTDN90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('SWTDN 10 ptile','SWTDN 50 ptile','SWTDN 70 ptile','SWTDN 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='SWTDN';
ylabel('Monthly Toa Incoming ShortWave Flux Average-W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('SWDTN','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' SWDTN Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of TOA SW incoming flux at all levels.';
