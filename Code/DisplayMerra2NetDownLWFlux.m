function  DisplayMerra2NetDownLWFlux(ikind,titlestr)
...
if(ikind==1)
    LWGNFluxT=LWGNTICES.values;
    LWGNFlux=LWGNFluxT(:,:,iTimeSlice);
    vmax=LWGNTICES.vmax;
    vmin=LWGNTICES.vmin;
    minval=-200;
    maxval=200;
...
%% Plot the Net Downward LW Flux so remove bad values
[nrows,ncols]=size(LWGNFlux);
numpts=nrows*ncols;
nanval=FillVal;
[LWGNFluxAdj,numNan] = RemoveNaNValues(LWGNFlux,nanval);
% Define a new array where all values less than 0 are set to 0 the
%values above 1 are set to 1 all others are untouched
[ibad,jbad]=find((LWGNFlux<vmin) | (LWGNFlux>vmax));
numbad=length(ibad);
ilow=0;
ihigh=0;
if(numbad>0)
    for jj=1:numbad
        indx=ibad(jj,1);
        indy=jbad(jj,1);
        nowval=LWGNFluxAdj(indx,indy);
        if(nowval<0)
            LWGNFluxAdj(indx,indy)=0;
            ilow=ilow+1;
        elseif(nowval>1)
            LWGNFluxAdj(indx,indy)=0;
            ihigh=ihigh+1;
...
nanfrac=numNan/numpts;
LWGNFluxAdjwMean=LWGNFluxAdj;
startmean=mean(mean(LWGNFluxAdj));
% Calculate some stats
LWGNFlux1D=reshape(LWGNFluxAdj,nrows*ncols,1);
LWGNFlux1DS=sort(LWGNFlux1D);
numpts1=length(LWGNFlux1D);
numpts2=numpts1-numNan;
LWGNFlux1DwMean=reshape(LWGNFluxAdjwMean,nrows*ncols,1);
LWGNFlux1DSwMean=sort(LWGNFlux1DwMean);
num01=floor(.01*numpts2);
num25=floor(.25*numpts2);
num50=floor(.50*numpts2);
num75=floor(.75*numpts2);
num99=floor(.99*numpts2);
val01=LWGNFlux1DS(num01,1);
val25=LWGNFlux1DS(num25,1);
val50=LWGNFlux1DS(num50,1);
val75=LWGNFlux1DS(num75,1);
val99=LWGNFlux1DS(num99,1);
val01wMean=LWGNFlux1DSwMean(num01,1);
val25wMean=LWGNFlux1DSwMean(num25,1);
val50wMean=LWGNFlux1DSwMean(num50,1);
val75wMean=LWGNFlux1DSwMean(num75,1);
val99wMean=LWGNFlux1DSwMean(num99,1);
headerstr=strcat('Basic Stats wMeans follow for-',desc,'-Data');
fprintf(fid,'%s\n',headerstr);
ptc1str=strcat('01 % LGWNFlux Value=',num2str(val01wMean,6));
fprintf(fid,'%s\n',ptc1str);
ptc25str=strcat('25 % LGWNFlux Value=',num2str(val25wMean,6));
fprintf(fid,'%s\n',ptc25str);
ptc50str=strcat('50 % LGWNFlux Value=',num2str(val50wMean,6));
fprintf(fid,'%s\n',ptc50str);
ptc75str=strcat('75 % LGWNFlux Value=',num2str(val75wMean,6));
fprintf(fid,'%s\n',ptc75str);
ptc99str=strcat('99 % LGWNFlux Vallue=',num2str(val99wMean,6));
fprintf(fid,'%s\n',ptc99str);
numbadstr=strcat('NumBadFrac=',num2str(numbadfrac,6),'-nanFrac-',num2str(nanfrac),...
    '-Array Mean Value=',num2str(startmean,6));
fprintf(fid,'%s\n',numbadstr);
