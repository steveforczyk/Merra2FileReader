function PlotLWUPTCLRTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function will plot the LWUPTCLR Table Values
...
plot(LWTUPCLRTT.Time,LWTUPCLRTT.LWTUPCLR10,'b',LWTUPCLRTT.Time,LWTUPCLRTT.LWTUPCLR50,'g',...
    LWTUPCLRTT.Time,LWTUPCLRTT.LWTUPCLR70,'c',LWTUPCLRTT.Time,LWTUPCLRTT.LWTUPCLR90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('LWTUPCLR 10 ptile','LWTUPCLR 50 ptile','LWTUPCLR 70 ptile','LWTUPCLR 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='LWTUPCLR';
ylabel('Monthly LW clear sky upwelling toa flux -W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('LWTUPCLR','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' LWTUPCLR Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of the long wave clear sky upwelling dlux at the toa.';
