function PlotHighCloudTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
...
plot(CLDHGHTT.Time,CLDHGHTT.CLDHGH10,'b',CLDHGHTT.Time,CLDHGHTT.CLDHGH50,'g',...
    CLDHGHTT.Time,CLDHGHTT.CLDHGH70,'c',CLDHGHTT.Time,CLDHGHTT.CLDHGH90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('HGH CLD Frac 10 ptile','HGH CLD Frac 50 ptile','HGH CLD Frac 70 ptile','HGH CLD Frac 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabel('Monthly High Cloud Frac Average','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('HighCloud','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('High Cloud Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average fractional cloud cover at high altitude (about 15 km).';
