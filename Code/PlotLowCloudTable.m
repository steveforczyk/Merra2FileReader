function PlotLowCloudTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function is designed to plot the Low Cloud Fraction Table Values
%  
%
...
plot(CLDLOWTT.Time,CLDLOWTT.CLDLOW10,'b',CLDLOWTT.Time,CLDLOWTT.CLDLOW50,'g',...
    CLDLOWTT.Time,CLDLOWTT.CLDLOW70,'c',CLDLOWTT.Time,CLDLOWTT.CLDLOW90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('LOW CLD Frac 10 ptile','LOW CLD Frac 50 ptile','LOW CLD Frac 70 ptile','LOW CLD Frac 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabel('Monthly LOW Cloud Frac Average','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('LowCloud','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('Low Cloud Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average fractional cloud cover at Low altitude (about 5 km).';
