function PlotTAUHGHTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function is plots the TTAUHGH Table Values
% 
%
...
global SWTDNTable SWTDNTT TAUHGHTable TAUHGHTT TSTable TSTT;
...
plot(TAUHGHTT.Time,TAUHGHTT.TAUHGH10,'b',TAUHGHTT.Time,TAUHGHTT.TAUHGH50,'g',...
    TAUHGHTT.Time,TAUHGHTT.TAUHGH70,'c',TAUHGHTT.Time,TAUHGHTT.TAUHGH90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('TAUHGH 10 ptile','TAUHGH 50 ptile','TAUHGH 70 ptile','TAUHGH 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='TAUHGH';
ylabel('TAUHGH','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('TAUHGH','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' TAUHGH Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of high clouds optical thickness.';
    parastr3='The expected data range for this dataset is from 0 to 500 and is in deg-k .';
