function PlotSWTNTCLRTable(titlestr,ikind,iAddToReport,iNewChapter,iCloseChapter)
% This function is the first attempt at plotting the SWDTN Table Values
% 
%
...
plot(SWTNTCLRTT.Time,SWTNTCLRTT.SWTNTCLR10,'b',SWTNTCLRTT.Time,SWTNTCLRTT.SWTNTCLR50,'g',...
    SWTNTCLRTT.Time,SWTNTCLRTT.SWTNTCLR70,'c',SWTNTCLRTT.Time,SWTNTCLRTT.SWTNTCLR90,'r');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gca,'FontWeight','bold');
set(gca,'XGrid','on','YGrid','on');
hl=legend('SWTNTCLR 10 ptile','SWTNTCLR 50 ptile','SWTNTCLR 70 ptile','SWTNTCLR 90 ptile');
ht=title(titlestr);
xlabel('Date','FontWeight','bold','FontSize',12);
ylabelstr='SWTNTCLR';
ylabel('Monthly Toa Net DownwardsShortWave Clear Sky Flux Average-W/m^2','FontWeight','bold','FontSize',12);
% Add a logo
if(iLogo==1)
...
if((iCreatePDFReport==1) && (RptGenPresent==1)  && (iAddToReport==1))
    ...
    sectionstr=strcat('SWTNTCLR','-Map');
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,~]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat(' SWTNTCLR Map For File-',Merra2ShortFileName);
...
% variable being plotted
    ...
    parastr2='This metric is a monthly average of TOA Net Downward SW Clear Sky flux at all levels.';
